...
> **스키마 변경 시 주의.**
> - Alembic을 의도적으로 생략했다 (§설계 의사결정). 앱이 뜰 때 `Base.metadata.create_all`이 *없는 테이블만* 만든다.
> - 즉 컬럼 추가나 제약 변경은 자동 반영되지 않는다. 모델을 바꿨다면 `docker compose down -v` 후 재기동해야 한다.
> - 운영 단계로 넘어가면 마이그레이션 1개를 추가하는 게 첫 단계가 된다.
...
신고 1건당 두 카운터가 각각 +1 된다.

- consumer 컨테이너의 durable subscription 두 개(`triage-metrics-triaged`, `triage-metrics-routed`)가 메시지를 ack한다.
- ack한 메시지는 `event_metrics` 테이블에 카운터로 누적된다.
- 즉 "publish만 한 게 아니라 후속 소비자가 실제로 읽었다"는 사실을 운영 화면 한 줄로 확인할 수 있다.

### 9. 운영자 큐 브라우저 (프론트)

브라우저에서 [http://localhost:3000](http://localhost:3000) 접속 → 4개 큐 탭(`fraud-review` / `spam-review` / `abuse-review` / `general-review`) 중 하나를 선택하면 큐별 분류 결과가 표로 보인다.

얇은 데모용 Next.js 한 페이지로, 내부적으로는 `GET /queues/{queue}/reports`만 호출한다 (인증·필터·페이지네이션 UI 없음).
...
- `Idempotency-Key` 헤더를 보내면 같은 키의 두 번째 요청은 새 row를 만들지 않고 기존 `report_id`를 그대로 돌려준다.
- 의도: 클라이언트가 응답을 유실해 retry할 때 중복 신고가 생기지 않게 한다.
- 헤더가 없으면 매 요청 새 row를 만든다 (기존 동작 유지).
- 동시 요청 race는 DB unique 제약 + IntegrityError 재조회로 흡수한다.
...
- **NATS JetStream은 "후속 시스템 경계".**
  - 핵심 처리 엔진이 아니라, 다운스트림 소비자가 FastAPI를 호출하지 않고도 결과를 받아갈 수 있게 하는 비동기 경계로 쓴다 (NFR-3).
  - 그 경계를 프로세스 경계로도 실체화하려고 후속 소비자(`consumer`)는 worker와 별도 컨테이너로 분리했다.
  - durable subscription 두 개(`triaged`, `routed`)가 메시지를 받아 `event_metrics` 테이블에 카운터를 누적한다.

- **`reports.status`가 단일 진실 원본.**
  - API는 Temporal workflow execution 상태를 직접 보지 않는다. activity가 DB에 상태를 기록하고, API는 DB만 읽는다 (`docs/03-product-requirements.md` FR-2).
  - workflow 시작 자체가 실패하면 status를 `workflow_start_failed`로 명시한다.
  - 덕분에 운영자가 GET 한 번으로 "분류 자체가 실패한 것"인지 "workflow가 시작조차 못 한 것"인지 구분할 수 있다.

- **NATS publish는 best-effort + publisher-side dedup.**

  *정책.* classification과 queue_item이 DB에 commit된 뒤의 `publish_triage_events_activity` 실패는 workflow에서 격리해 잡고, 그대로 `mark_classified`로 진행한다 (재시도는 `_DEFAULT_RETRY`로 3회까지 시도하고 포기).

  *왜 best-effort인가.* NATS는 "후속 시스템 경계"이지 핵심 처리 엔진이 아니다. downstream 카운터가 잠시 비는 것보다, `reports.status`를 `failed`로 되돌려 "분류 자체가 실패한 것처럼" 보이는 쪽이 운영자에게 더 큰 혼선이다.

  *중복 발행 방지.* `publish_triaged → publish_queue_routed` 두 publish 중 한쪽이 실패해 워크플로가 retry되면 같은 메시지가 두 번 나갈 수 있다. 이를 막기 위해:
  - publisher가 `Nats-Msg-Id: {subject}:{report_id}` 헤더를 붙인다.
  - JetStream stream에 `duplicate_window=120s`를 설정한다.

  *관찰·확장.* 누락된 publish는 `/metrics/events`로 관찰할 수 있고, 운영성이 더 중요해지면 outbox 패턴으로 강화한다.

- **분류는 규칙 기반.** 정확도보다 운영 가능한 흐름을 우선한다. classifier 인터페이스는 `(reason_code, description, metadata) -> ClassificationResult` 한 줄짜리라 추후 LLM으로 교체할 수 있다.
...
- **재처리는 upsert-overwrite + active workflow ≤ 1.**
  - classification과 queue item은 `report_id` 기준으로 최신 1건만 유지하고, 이력 테이블은 두지 않는다.
  - workflow_id는 모든 run에 대해 `report-triage-{report_id}` 단일 ID를 사용한다.
  - 새 run을 시작할 때 Temporal `WorkflowIDReusePolicy.TERMINATE_IF_RUNNING`이 기존 running run을 자동으로 종료한다.
  - 결과는 새 run의 결과로 덮어써지고, stale run의 늦은 write는 cancellation으로 차단된다.
  - API 게이트는 두지 않는다 — 어떤 상태에서도 운영자가 즉시 재분류할 수 있다 (`docs/06-workflow-and-events.md` §workflow_id 전략).
...
- **`/metrics/events`가 빈 배열로 나옴**
  - consumer 컨테이너가 안 떠 있을 가능성이 크다.
  - `docker compose ps consumer`로 상태를 확인하고, `docker compose logs consumer --tail 30`으로 원인을 본다.
  - publish는 정상이지만 consumer만 죽어 있는 경우, 메시지는 JetStream에 쌓여 있다가 consumer가 다시 뜨면 한 번에 드레인된다.

- **첫 빌드 직후 worker가 `temporal:7233 connection refused`로 재시작 루프**
  - Temporal `auto-setup`이 Postgres 스키마를 구성하는 데 ~15초 걸리고, 그 사이 7233은 아직 listen 전이다.
  - compose healthcheck가 `start_period: 20s`로 흡수하도록 해뒀지만, worker가 먼저 떴다면 잠깐 기다리거나 `docker compose restart worker`로 해결한다.
